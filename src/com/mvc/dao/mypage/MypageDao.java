...
public interface MypageDao {
	...
	String selectTravelSql = " SELECT * FROM V_BLOG_LIST WHERE USER_ID=? "; // 내 여행 조회
	//String deleteTravelSql = " DELETE FROM V_BLOG_LIST WHERE USER_ID=? AND BLOG_SEQ=? "; // 내 여행 삭제

	String selectWishedBSql = " SELECT * FROM BLOG_HEART WHERE USER_ID=? "; //블로그찜 조회
	...
	String selectCompanionSql = " SELECT * FROM M_PROMISE WHERE SEN_ID=? OR REC_ID=?"; // 동행 조회
